...
void ABossTonado::BeginPlay()
{
	...
	StatData = myGameInstace->GetTonadoData();
...
void ABossTonado::Tick(float DeltaTime)
{
	...
	if (attackTimer <= 0.0f)
	{
		attackTimer += StatData->AttackTime;
		...
		bool bResult = GetWorld()->SweepMultiByChannel(
			HitResults,
			GetActorLocation(),
			GetActorLocation() + GetActorForwardVector() * StatData->Radius,
			FQuat::Identity,
			ECollisionChannel::ECC_Pawn,
			FCollisionShape::MakeSphere(StatData->Radius),
			Params);
		...
		if (bResult)
		{
			for (auto HitResult : HitResults) {
				if (HitResult.Actor.IsValid())
				{
					...
					HitResult.Actor->TakeDamage(StatData->Damage, DamageEvent, nullptr, this);
				}
			}
		}
	}
	...
	newVec.Normalize();
	ProjectileMovementComponent->Velocity = newVec * StatData->Speed;
